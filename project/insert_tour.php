...
if (isset($_POST['insert_tour'])) {
    ...
    $tour_international = test_input($_POST['International']);
    $tour_local = test_input($_POST['local']);

    //getting image from the field
    $tour_image_tmp = $_FILES['tour_image']['tmp_name'];
    $tour_image_size = $_FILES['tour_image']['size'];
    ...
    if (!preg_match("/[a-zA-Z0-9]+/", $tour_id) || strlen($tour_id) < 2) {
    ...
    } else if ($tour_local == "Select Category") {
        $response = array(
            "type" => "warning",
            "message" => "Select tour category."
        );
    } else if (file_exists($tour_image_tmp)) {

        $image_info = getimagesize($tour_image_tmp);
        $width = $image_info[0];
        $height = $image_info[1];
        $target_directory = "tour_images/";
        $allowed_image_extension = array("png", "jpg", "jpeg");

        // Get image file extension
        $image_extension = pathinfo($tour_image_name, PATHINFO_EXTENSION);
        ...
        if (!in_array($image_extension, $allowed_image_extension)) {
        ...
        else if ($tour_image_size > 2000000) {
            $response = array(
                "type" => "warning",
            ...
            if (move_uploaded_file($tour_image_tmp, $target)) {

                $insert_tour = "insert into tours (tour_id,International,local) 
                  VALUES ('$tour_id','$tour_international','$tour_local');";
                $insert_tour = mysqli_query($con, $insert_tour);
                if ($insert_tour) {
