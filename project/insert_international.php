...
if (isset($_POST['insert_international'])) {
    //getting text data from the fields
    $tour_id = test_input($_POST['tour_id']);
    $tour_international = test_input($_POST['country_name']);

    //getting image from the field
    $international_image_tmp = $_FILES['international_image']['tmp_name'];
    $international_image_size = $_FILES['international_image']['size'];

    if (!preg_match("/[a-zA-Z0-9]+/", $tour_id) || strlen($tour_id) < 2) {
        $response = array(
            "type" => "warning",
            "message" => "Enter Valid tour_id."
        );
    } else if ($tour_international == "Select Category") {
        $response = array(
            "type" => "warning",
            "message" => "Select tour Category."
        );
    } else if (file_exists($international_image_tmp)) {

        $image_info = getimagesize($international_image_tmp);
        $width = $image_info[0];
        $height = $image_info[1];
        $target_directory = "international_images/";
        $allowed_image_extension = array("png", "jpg", "jpeg");

        // Get image file extension
        $image_extension = pathinfo($international_image_name, PATHINFO_EXTENSION);
        ...
        if (!in_array($image_extension, $allowed_image_extension)) {
        ...
        else if ($international_image_size > 2000000) {
            $response = array(
                "type" => "warning",
            ...
            if (move_uploaded_file($international_image_tmp, $target)) {

                $insert_international = "insert into international (tour_id,country_name) 
                  VALUES ('$tour_id','$country_name');";
                $insert_international = mysqli_query($con, $insert_international);
                if ($insert_international) {
                    //header("location: ".$_SERVER['PHP_SELF']);
                    $response = array(
                        "type" => "success",
                        "message" => "International Tour uploaded successfully."
                    );
                }
