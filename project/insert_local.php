...
if (isset($_POST['insert_local'])) {
    //getting text data from the fields
    $tour_id = test_input($_POST['tour_id']);
    $tour_desert = test_input($_POST['desert']);
    $tour_northern = test_input($_POST['northern']);

    //getting image from the field
    $local_image_tmp = $_FILES['local_image']['tmp_name'];
    $local_image_size = $_FILES['local_image']['size'];
    ...
    if (!preg_match("/[a-zA-Z0-9]+/", $tour_id) || strlen($tour_id) < 2) {
    ...
    } else if ($tour_desert == "Select Category") {
        $response = array(
            "type" => "warning",
            "message" => "Select tour Category."
        );
    } else if ($tour_northern == "Select Category") {
        $response = array(
            "type" => "warning",
            "message" => "Select tour Category."
        );
    } else if (file_exists($local_image_tmp)) {

        $image_info = getimagesize($local_image_tmp);
        $width = $image_info[0];
        $height = $image_info[1];
        $target_directory = "local_images/";
        $allowed_image_extension = array("png", "jpg", "jpeg");

        // Get image file extension
        $image_extension = pathinfo($local_image_name, PATHINFO_EXTENSION);
        ...
        if (!in_array($image_extension, $allowed_image_extension)) {
        ...
        else if ($local_image_size > 2000000) {
            $response = array(
                "type" => "warning",
            ...
            if (move_uploaded_file($local_image_tmp, $target)) {

                $insert_local = "insert into local (tour_id,desert,northern) VALUES ('$tour_id','$desert','$northern');";
                $insert_local = mysqli_query($con, $insert_local);
                if ($insert_local) {
